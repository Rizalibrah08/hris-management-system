...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, ScrollView, TextInput, KeyboardAvoidingView, Platform, Modal, Alert, ActivityIndicator } from 'react-native';
...
export default function SubmitLeaveScreen() {
  ...
  const handleSubmitLeave = async () => {
    if (!selectedCategory || !rangeStart || !rangeEnd) {
      Alert.alert('Error', 'Kategori dan rentang tanggal wajib diisi');
      return;
    }
    ...
    try {
      ...
      setTimeout(() => { setIsSubmitSuccessVisible(true); }, 300);
    } catch (err) {
      Alert.alert('Gagal', err.message || 'Gagal mengajukan cuti');
    } finally {
  ...
  const getCalendarDays = () => {
    ...
    for (let row = 0; row < 6; row++) {
      ...
      for (let col = 0; col < 7; col++) {
        ...
        if (cellIndex < firstDay && row === 0) {
          const prevDay = daysInPrevMonth - firstDay + col + 1;
          week.push({ d: String(prevDay), prev: true });
        } else if (currentDay > daysInMonth) {
          week.push({ d: String(nextMonthDay++), next: true });
          finished = true;
        } else {
          started = true;
          ...
          if (startNum && !endNum && dayNum === startNum) isStandalone = true;

          week.push({ d: String(dayNum), start, end, mid, isStandalone });
          currentDay++;
        }
      }
      if (finished && currentDay > daysInMonth) {
        rows.push(week);
        break;
      }
      rows.push(week);
...
<TouchableOpacity 
                ...
                >
                  {submitLoading ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.btnPrimaryText}>Yes, Submit</Text>}
                </TouchableOpacity>
                <TouchableOpacity 
                  style={styles.btnSecondaryFull} 
                  onPress={() => setIsSubmitConfirmVisible(false)}
